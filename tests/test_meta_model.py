...
from app.db.models.meta import Meta  # noqa: F401  (import ensures model is registered)
...
def test_meta_table_can_be_created(monkeypatch):
    """
    Basic integration test to ensure the Meta model is wired correctly and
...
def test_meta_table_has_expected_columns(monkeypatch):
    """
    Ensure the Meta table exposes the expected column names, including
    ...
    column_names = {col["name"] for col in inspector.get_columns("meta")}
