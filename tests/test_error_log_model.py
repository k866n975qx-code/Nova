...
from app.db.models.error_log import ErrorLog  # noqa: F401  (import ensures model is registered)
...
def test_error_log_table_can_be_created(monkeypatch):
    """
    Basic integration test to ensure the ErrorLog model is wired correctly and
...
def test_error_log_table_has_expected_columns(monkeypatch):
    """
    Ensure the error_log table exposes the expected column names, including
    ...
    column_names = {col["name"] for col in inspector.get_columns("error_log")}
