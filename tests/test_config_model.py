...
from app.db.models.config import Config  # noqa: F401  (import ensures model is registered)
...
def test_config_table_can_be_created(monkeypatch):
    """
    Basic integration test to ensure the Config model is wired correctly and
...
def test_config_table_has_expected_columns(monkeypatch):
    """
    Ensure the config table exposes the expected column names, including
    ...
    column_names = {col["name"] for col in inspector.get_columns("config")}
