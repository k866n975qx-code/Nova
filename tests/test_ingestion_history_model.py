...
from app.db.models.ingestion_history import IngestionHistory  # noqa: F401  (import ensures model is registered)
...
def test_ingestion_history_table_can_be_created(monkeypatch):
    """
    Basic integration test to ensure the IngestionHistory model is wired correctly and
...
def test_ingestion_history_table_has_expected_columns(monkeypatch):
    """
    Ensure the ingestion_history table exposes the expected column names, including
    ...
    column_names = {col["name"] for col in inspector.get_columns("ingestion_history")}
